...
# This value dictates how much USD is passively generated by the World Debt Node (converted to Capsule MB)
...
def load_wallet(wallet_id):
    ...
    # Ensure all new fields are present as Decimal for arithmetic consistency
...
def world_debt_node_value_generation():
    ...
    # Ensure this function only runs for the correct debt node (security/logic check)
    if debt_wallet.get('node_id') != WORLD_DEBT_NODE_ID:
        return
        
    # Convert USD passive gain to the storable Capsule MB
...
def unified_mining_loop(wallet, mining_type):
    ...
    try:
        while current_tick < MAX_TICKS:
            ...
            # Passive Hash Power Growth is calculated on the (potentially) boosted total
...
# --- Send and Donate Functions (Kept as is) ---
...
# --- Utility Functions for Wallet Selection (Kept as is) ---
...
def select_wallet_or_rig():
    ...
    # If a Wallet ID was typed, load it directly
...
def show_rig_dashboard(wallet):
    ...
    # This now shows the permanently boosted value.
...
# --- Wallet Transactions & Donations (Updated) ---
...
def start_mining(mining_type):
    # Uses the new numbered selection function
