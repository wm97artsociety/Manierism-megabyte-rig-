...
def mint_capsule(wallet, capsule_type, reward_mb, mining_type):
    ...
    # --- Hash power gain for all mining types ---
    hash_gain = reward_mb * Decimal("0.01")
    wallet["rig_hash_power"] += hash_gain

    # SHA extra boost (once per session)
    sha_boost = Decimal("0")
    if mining_type == "sha":
...
def unified_mining_loop(wallet, mining_type):
    ...
    try:
        while True:
            ...
            # Print reward with emojis, double-spaced
    ...
    while True:
        ...
        if option == "1":
            ...
            if wallet["capsule_value_mb"] >= amt and donation_wallet:
                ...
                print(f"🌠 Hash Power increased by donation: +{amt*Decimal('0.1'):.6f} H/s")
                save_wallet(wallet)
